...
export default async function WatchPage({ params }: { params: { id: string } }) {
  ...
  // --- Fetch player link from Kodik API ---
  let playerSrc: string | null = null
  let kodikError: string | null = null

  // Для простоты, берем первую доступную озвучку и первый эпизод
  const selectedTranslation = translations[0]
  const selectedEpisode = 1 // Предполагаем, что начинаем с эпизода 1

  if (selectedTranslation) {
    try {
      const kodikToken = process.env.KODIK_API_TOKEN
      if (!kodikToken) {
        throw new Error("KODIK_API_TOKEN is not set in environment variables.")
      }

      // Kodik API endpoint для получения ссылки на плеер
      // link_id из таблицы translations соответствует link_id в Kodik API
      const kodikApiUrl = `https://kodikapi.com/get_player_link?token=${kodikToken}&link_id=${selectedTranslation.link_id}`
      const response = await fetch(kodikApiUrl, { next: { revalidate: 3600 } }) // Ревалидация каждый час
      const data = await response.json()

      if (response.ok && data.link) {
        playerSrc = data.link
      } else {
        kodikError = data.message || "Не удалось получить ссылку на плеер от Kodik API."
        console.error("Kodik API error:", data)
      }
    } catch (error: any) {
      kodikError = `Ошибка при получении ссылки на плеер: ${error.message}`
      console.error("Error fetching Kodik player link:", error)
    }
  }

  // Если ссылка на плеер не найдена
  if (!playerSrc) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center justify-center min-h-[60vh]">
            <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm max-w-md w-full">
              <CardContent className="p-8 text-center">
                ...
                <h2 className="text-xl font-bold text-white mb-2">Плеер недоступен</h2>
                <p className="text-slate-400 mb-6">
                  {kodikError || "К сожалению, не удалось загрузить плеер для этого аниме."}
                </p>
                <Link href={`/anime/${params.id}`}>
                  <Button
                    variant="outline"
            ...
            <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm overflow-hidden">
              <CardContent className="p-0">
                <div className="aspect-video bg-black relative group">
                  <PlayerClient src={playerSrc} poster={animeData.poster_url || undefined} />
            ...
            <Card className="bg-slate-800/50 border-slate-700 backdrop-blur-sm">
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h2 className="text-xl font-bold text-white mb-2">Эпизод {selectedEpisode}</h2>
                    <p className="text-slate-400">{selectedTranslation?.title || "Неизвестная озвучка"}</p>
                  </div>
                  <div className="flex items-center space-x-2">
