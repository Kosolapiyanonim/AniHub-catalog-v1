...
## Ручной парсинг через админ-панель

Сценарий обновлён под ручной запуск:
- страница: `/admin/parser`;
- endpoint: `POST /api/manual-parser`;
- периоды: `24h`, `7d`, `30d`, `180d`;
- для дебага есть кнопки "Тест: 1 аниме" для каждого периода.

Аудит legacy-парсеров и план удаления: `docs/parser-audit.md`.
