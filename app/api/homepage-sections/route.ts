...
export async function GET() {
  ...
  try {
    ...
    const [heroAnimesResponse, trending, popular, recentlyCompleted, latestUpdates] = await Promise.all([
      supabase
        .from("animes")
      ...
      supabase
        ...
        .gte("updated_at_kodik", twoMonthsAgo.toISOString())
        .not("shikimori_id", "is", null)
      ...
      supabase
        ...
        .select(ANIME_CARD_SELECT)
        .not("shikimori_id", "is", null)
        .order("shikimori_votes", { ascending: false })
        .limit(12),
      supabase
        .from("animes_with_details")
        .select(ANIME_CARD_SELECT)
        .eq("status", "released")
        .gte("updated_at_kodik", oneMonthAgo.toISOString())
        .not("shikimori_id", "is", null)
        .order("weighted_rating", { ascending: false })
        .limit(12),
      supabase
        ...
        .select(ANIME_CARD_SELECT)
        .not("shikimori_id", "is", null)
        .order("updated_at_kodik", { ascending: false })
        .order("weighted_rating", { ascending: false })
        .limit(12),
    ])
    ...
    if (session) {
      ...
      const [continueWatchingResponse, myUpdatesResponse] = await Promise.all([
        supabase
          .from("user_lists")
          .select(`progress, animes!inner(${ANIME_CARD_SELECT})`)
          .eq("user_id", user.id)
          .eq("status", "watching")
          .not("animes.shikimori_id", "is", null)
          .order("updated_at", { ascending: false })
          .limit(6),
        supabase
          .from("user_subscriptions")
          .select(`animes!inner(${ANIME_CARD_SELECT})`)
          .eq("user_id", user.id)
          .not("animes.shikimori_id", "is", null)
      ...
      continueWatching =
        continueWatchingResponse.data
          ?.map((item) => (item.animes ? { ...item.animes, progress: item.progress } : null))
          .filter(Boolean) || []
