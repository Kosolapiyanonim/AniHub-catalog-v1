...
export function CatalogFilters({ initialFilters, onApply }: CatalogFiltersProps) {
  ...
  const [tags, setTags] = useState<FilterItem[]>([]) // Добавлено, но не используется в этом фрагменте
        ...
        <Accordion type="multiple" className="w-full" defaultValue={["user_list_status", "genres"]}>
          {session && (
            <FilterSection title="Мои списки">
              <Select
                ...
                onValueChange={(val) => setFilters((prev) => ({ ...prev, user_list_status: val === "all" ? "" : val }))}
              >
                <SelectTrigger className="w-full bg-slate-700 border-slate-600">
                  <SelectValue placeholder="Выберите список" />
                </SelectTrigger>
                <SelectContent className="bg-slate-800 border-slate-700 text-white">
                  {userListStatuses.map((status) => (
          ...
          <FilterSection title="Жанры">
            <MultiSelectFilter
              ...
              onChange={(s, e) => setFilters((prev) => ({ ...prev, genres: s, genres_exclude: e }))}
            />
          </FilterSection>
          ...
          <FilterSection title="Тип">
            <CheckboxGroup
              items={animeKinds}
              selected={filters.kinds} // <-- ИЗМЕНЕНИЕ
              onChange={(val) => setFilters((prev) => ({ ...prev, kinds: val }))} // <-- ИЗМЕНЕНИЕ
            />
          </FilterSection>
          ...
          <FilterSection title="Студии">
            <MultiSelectFilter
              ...
              excluded={filters.studios_exclude}
              onChange={(s, e) => setFilters((prev) => ({ ...prev, studios: s, studios_exclude: e }))}
            />
          </FilterSection>
          <FilterSection title="Теги">
            <MultiSelectFilter
              ...
              excluded={filters.tags_exclude}
              onChange={(s, e) => setFilters((prev) => ({ ...prev, tags: s, tags_exclude: e }))}
            />
          </FilterSection>
        </Accordion>
      </CardContent>
...
const MultiSelectFilter = ({ items, selected, excluded, onChange }: any) => {
  const handleStateChange = (slug: string, currentState: "none" | "included" | "excluded") => {
    ...
    if (currentState === "none") {
      ...
      newExcluded.delete(slug)
    } else if (currentState === "included") {
      newSelected.delete(slug)
      newExcluded.add(slug)
    } else {
      newExcluded.delete(slug)
    }
        ...
        {items.map((item: FilterItem) => {
          ...
          return (
            <Button
              ...
              onClick={() => handleStateChange(`${item.id}-${item.slug}`, state)}
            >
              <div
                className={`w-4 h-4 rounded-sm border border-primary flex items-center justify-center ${isIncluded ? "bg-primary" : ""} ${isExcluded ? "bg-destructive" : ""}`}
              >
    ...
    <div className="space-y-2">
      {items.map((item) => (
        <div key={item} className="flex items-center space-x-2">
          ...
          <Label htmlFor={item} className="text-sm font-medium text-gray-300 capitalize">
            {item.replace(/_/g, " ")}
          </Label>
        </div>
      ))}
