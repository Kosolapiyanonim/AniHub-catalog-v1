...
export function Header() {
    ...
    // Эффект для закрытия дропдауна при клике вне его области
    useEffect(() => {
    ...
    // Обработчик для перехода на страницу каталога при нажатии Enter
            ...
            <div className="container mx-auto px-4">
                <div className="flex items-center justify-between h-16">
                    ...
                    <nav className="hidden md:flex items-center space-x-8">
                        <Link href="/" className="text-gray-300 hover:text-white transition-colors">Главная</Link>
                        <Link href="/catalog" className="text-gray-300 hover:text-white transition-colors">Каталог</Link>
                        <Link href="/popular" className="text-gray-300 hover:text-white transition-colors">Популярное</Link>
                    </nav>
                        ...
                        <form onSubmit={handleSearchSubmit} className="flex items-center">
                            <div className="relative w-64">
                                ...
                                <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                                    {isSearching ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="w-4 h-4 text-gray-400" />}
                                </div>
                            </div>
                        </form>
