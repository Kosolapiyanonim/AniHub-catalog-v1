...
export async function POST(request: Request) {
    ...
    try {
        ...
        const { data: upsertedAnimes, error: animeError } = await supabase
            .from('animes')
            .upsert(animeRecordsToUpsert, { onConflict: 'shikimori_id' }) // <-- ИСПРАВЛЕНИЕ: Конфликт по shikimori_id
        ...
        const allTranslations = animeList.map(anime => {
            ...
            return {
                anime_id,
                kodik_id: anime.id, // <-- ПРАВИЛЬНО: ID озвучки от Kodik
        ...
        if(allTranslations.length > 0) {
            await supabase.from('translations').upsert(allTranslations, { onConflict: 'kodik_id' }); // <-- ИСПРАВЛЕНИЕ: Конфликт по kodik_id
        }
