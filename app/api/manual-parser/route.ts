...
export async function GET() {
  return NextResponse.json({
    ...
    supportedAnimeIdTypes: ["shikimori", "internal"],
    bodyExample: {
      period: "24h",
      animeId: "50738",
      animeIdType: "shikimori",
      dryRun: false,
...
export async function POST(request: Request) {
  ...
  const animeIdType = (body.animeIdType as AnimeIdType) || "shikimori";
  ...
  let targetShikimoriId = "";
  if (animeId) {
    if (animeIdType === "shikimori") {
      targetShikimoriId = animeId;
      addLog("INIT", `Режим точечного парсинга по shikimori_id=${targetShikimoriId}`);
    } else {
      const internalId = Number(animeId);
      if (!Number.isFinite(internalId)) {
        return NextResponse.json({ error: "internal animeId должен быть числом" }, { status: 400 });
      }

      const { data: found, error: foundError } = await supabase
        .from("animes")
        .select("id, shikimori_id, title")
        .eq("id", internalId)
        .maybeSingle();

      if (foundError) {
        return NextResponse.json({ error: `Ошибка поиска internal anime id: ${foundError.message}` }, { status: 500 });
      }
      if (!found?.shikimori_id) {
        return NextResponse.json({ error: `Аниме с internal id=${animeId} не найдено или не имеет shikimori_id` }, { status: 404 });
      }

      targetShikimoriId = found.shikimori_id;
      addLog("INIT", `internal id=${animeId} -> shikimori_id=${targetShikimoriId}, title=${found.title || "unknown"}`);
    }
  }
  ...
  try {
    while (pagesScanned < maxPages && !reachedThreshold) {
      ...
      if (!nextPageUrl) {
        ...
        if (targetShikimoriId) {
          targetUrl.searchParams.set("shikimori_id", targetShikimoriId);
          addLog("FETCH", `Применён фильтр shikimori_id=${targetShikimoriId}`);
        }
      }
      ...
      for (const anime of pageItems) {
        ...
        if (targetShikimoriId && anime.shikimori_id !== targetShikimoriId) {
          addLog("FILTER", `Пропуск чужого тайтла в режиме ID: ${anime.title} (${anime.shikimori_id})`);
          continue;
        }

        const updatedAt = parseDate(anime.updated_at);
        if (!targetShikimoriId && updatedAt && updatedAt < threshold) {
      ...
      if (targetShikimoriId && animeMap.size >= 1) {
        addLog("FETCH", "Режим по ID: запись найдена, дальнейший обход не нужен.");
        break;
      }
    }
    ...
    if (dryRun) {
      ...
      return NextResponse.json({
        ...
        singleAnimeDebug,
        targetShikimoriId: targetShikimoriId || null,
        logs,
      });
    }
    ...
    for (const anime of uniqueAnimeList) {
      const animeIdDb = idMap.get(anime.shikimori_id!);
      if (!animeIdDb) {
        addLog("RELATION", `Пропуск связей: не найден anime_id в БД для ${anime.title} (${anime.shikimori_id})`);
        continue;
      }

      addLog("RELATION", `Обновление связей: ${anime.title} (${anime.shikimori_id}) -> anime_id=${animeIdDb}`);
      await processAllRelationsForAnime(supabase, anime, animeIdDb);
