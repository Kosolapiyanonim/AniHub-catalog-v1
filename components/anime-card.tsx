...
import { AddToListButton } from './AddToListButton'; // <-- [ИЗМЕНЕНИЕ] Импортируем нашу новую кнопку
import { Popover, PopoverContent, PopoverTrigger } from './ui/popover'; // <-- [ИЗМЕНЕНИЕ] Импортируем Popover
import { AnimeHoverCard } from './AnimeHoverCard'; // <-- [ИЗМЕНЕНИЕ] Импортируем контент для Hover
...
export function AnimeCard({ anime, priority = false }: AnimeCardProps) {
  ...
  return (
    // --- [ИЗМЕНЕНИЕ] Оборачиваем карточку в Popover для ховер-эффекта ---
    <Popover>
      <PopoverTrigger asChild>
        <div className="relative group">
          <AddToListButton animeId={anime.id} initialStatus={anime.user_list_status} variant="icon" />
      ...
      {/* --- [ИЗМЕНЕНИЕ] Добавляем контент, который будет показан при наведении --- */}
      <PopoverContent side="right" align="start" sideOffset={10} className="w-80 p-0 border-slate-700">
