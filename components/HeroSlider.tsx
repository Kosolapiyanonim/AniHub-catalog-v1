...
import React, { useRef } from "react" // <--- ИСПРАВЛЕНИЕ: Добавлен useRef
      ...
      <CarouselContent>
        {validItems.map((anime, index) => (
          <CarouselItem key={anime.id}>
            <div className="relative h-[70vh] w-full flex md:flex-row flex-col-reverse bg-slate-900 overflow-hidden rounded-lg">
              {/* Левая часть с информацией */}
              <div className="relative z-10 w-full md:w-3/5 lg:w-3/5 flex items-center p-4 sm:p-8 md:pl-16">
                <div className="text-white w-full max-w-lg">
                  ...
                  <div className="flex items-center gap-4">
                    <Button asChild size="lg" className="bg-purple-600 hover:bg-purple-700"><Link href={`/anime/${anime.shikimori_id}/watch`}><Play className="w-5 h-5 mr-2" />Смотреть</Link></Button>
                    <Button asChild size="lg" variant="outline"><Link href={`/anime/${anime.shikimori_id}`}><Info className="w-5 h-5 mr-2" />Подробнее</Link></Button>
                  </div>
                </div>
              </div>

              {/* Правая часть с постером */}
              <div className="w-full md:w-2/5 lg:w-2/5 h-1/2 md:h-full relative">
                <div className="absolute inset-0 bg-gradient-to-t md:bg-gradient-to-l from-slate-900 via-slate-900/50 to-transparent z-10"></div>
                {anime.poster_url && (
                    <Image
