...
-- 2. Для СУЩЕСТВУЮЩЕЙ базы с данными - используйте миграции из /scripts (например, add-ratings-system.sql)
...
DROP TABLE IF EXISTS user_episode_ratings CASCADE;
...
CREATE INDEX IF NOT EXISTS idx_user_anime_ratings_user ON user_anime_ratings(user_id, rating DESC, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_user_anime_ratings_anime ON user_anime_ratings(anime_id);
...
CREATE INDEX IF NOT EXISTS idx_user_episode_ratings_user ON user_episode_ratings(user_id, rating DESC, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_user_episode_ratings_anime ON user_episode_ratings(anime_id);
...
-- ============================================================================
-- Политики для user_anime_ratings
-- ============================================================================
DROP POLICY IF EXISTS "user_anime_ratings_select_owner" ON user_anime_ratings;
CREATE POLICY "user_anime_ratings_select_owner" ON user_anime_ratings FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "user_anime_ratings_insert_owner" ON user_anime_ratings;
CREATE POLICY "user_anime_ratings_insert_owner" ON user_anime_ratings FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "user_anime_ratings_update_owner" ON user_anime_ratings;
CREATE POLICY "user_anime_ratings_update_owner" ON user_anime_ratings FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "user_anime_ratings_delete_owner" ON user_anime_ratings;
CREATE POLICY "user_anime_ratings_delete_owner" ON user_anime_ratings FOR DELETE USING (auth.uid() = user_id);

-- ============================================================================
-- Политики для user_episode_ratings
-- ============================================================================
DROP POLICY IF EXISTS "user_episode_ratings_select_owner" ON user_episode_ratings;
CREATE POLICY "user_episode_ratings_select_owner" ON user_episode_ratings FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "user_episode_ratings_insert_owner" ON user_episode_ratings;
CREATE POLICY "user_episode_ratings_insert_owner" ON user_episode_ratings FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "user_episode_ratings_update_owner" ON user_episode_ratings;
CREATE POLICY "user_episode_ratings_update_owner" ON user_episode_ratings FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "user_episode_ratings_delete_owner" ON user_episode_ratings;
CREATE POLICY "user_episode_ratings_delete_owner" ON user_episode_ratings FOR DELETE USING (auth.uid() = user_id);
